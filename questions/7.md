...
* Q: 解释下原型继承的原理。
  ...
  原型链查找基本概念：
    ...
    5. (自己添加) 每一个对象都有一个一个原型对象 prototype, 可以通过new 关键字调用函数生成一个对象， 调用这个生成的对象的方法或者属性的时候， 如果没有在这个对象上找到对应的属性或者方法，就去 **构造函数** 的原型对象上面查找。
    6. 类的方法都定义在原型上，在类的实例上面调用函数就是调用构造函数的原型上的方法。
    7. 子类的__proto__属性，表示构造函数的继承，总是指向父类。
    8. 子类prototype属性的__proto__属性，表示方法的继承，总是指向父类的prototype属性。
```javascript
class A {
}
